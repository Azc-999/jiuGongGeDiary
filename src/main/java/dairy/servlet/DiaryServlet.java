...
import dairy.DBTool.PagingTool;
import dairy.dao.DiaryDao;



public class DiaryServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	PagingTool paging = null;	//分页工具类的对象
    DiaryDao dao = null;	//日记相关的数据库操作对象   
	
    public DiaryServlet() {
        super();
        dao = new DiaryDao();	//实例化对象
    }
	 ...
	 * @throws IOException
	 */
	private void delDiary(HttpServletRequest request,HttpServletResponse response) 
			throws ServletException, IOException {
	...
	private void listMyDiary(HttpServletRequest request,HttpServletResponse response) 
		...
		if (strPage == null) {
			...
			String sql = "select d.*,u.uname from dy_diary d inner join dy_user u "
					+ "on u.id=d.userid  where d.userid="+ userid + " order by d.writeTime DESC";
			paging = new PagingTool();
			list = dao.queryDiary(sql); // 获取日记内容
			int pagesize = 4; // 指定每页显示的记录数
			list = paging.getInitPage(list, Page, pagesize); // 初始化分页信息
			request.getSession().setAttribute("paging", paging);// 保存分页信息
		} else {
			paging = (PagingTool) request.getSession().getAttribute(
					"paging");// 获取分页信息
			Page = paging.getPage(strPage);
			list = paging.getAppointPage(Page); // 获取指定页数据
		}
	...
	public void listAllDiary(HttpServletRequest request,HttpServletResponse response) 
		...
		if (strPage == null) {// 当页面初次运行
			String sql = "select d.*,u.uname from dy_diary d inner join "
					+ "dy_user u on u.id=d.userid order by d.writeTime DESC limit 50";
			paging = new PagingTool();
			list = dao.queryDiary(sql); // 获取日记内容
			int pagesize = 4; // 指定每页显示的记录数
			list = paging.getInitPage(list, Page, pagesize); // 初始化分页信息
			...
			paging = (PagingTool) request.getSession().getAttribute(
					"paging");
			Page = paging.getPage(strPage);// 获取当前页码
			list = paging.getAppointPage(Page); // 获取指定页数据
		}
		request.setAttribute("diaryList", list); // 保存当前页的日记信息
		request.setAttribute("Page", Page); // 保存的当前页码
