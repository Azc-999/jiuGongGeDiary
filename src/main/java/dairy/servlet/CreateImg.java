...
public class CreateImg extends HttpServlet {
    ...
    public void service(HttpServletRequest request, HttpServletResponse response)
    	...
    	// 禁止缓存
    			...
    			if ("默认".equals(template)) {
    				bgImgFile = new File(request.getRealPath("images/bg_00.png"));
    				...
    				bgImgFile = new File(request.getRealPath("images/bg_01.png"));
    				Image src = ImageIO.read(bgImgFile); // 构造Image对象
    				g.drawImage(src, 0, 0, width, height, null); // 绘制背景图片
    				outWord(g, content, weather, 25, 110);

    			} else {
    				bgImgFile = new File(request.getRealPath("images/bg_02.png"));
    				Image src = ImageIO.read(bgImgFile); // 构造Image对象
    				g.drawImage(src, 0, 0, width, height, null); // 绘制背景图片
    ...
    public void outWord(Graphics g, String[] content, String weather,
		...
		Font mFont = new Font("黑体", Font.PLAIN, 26); // 通过Font构造字体
		...
		for (int i = 0; i < content.length; i++) {
			...
			if (content[i].equals("weathervalue")) {
				...
				mFont = new Font("黑体", Font.PLAIN, 14); // 通过Font构造字体
				g.setFont(mFont); // 设置字体
				Date date = new Date();
			...
			if (contentLen < 5) {
				switch (contentLen % 5) {
				case 1:
					mFont = new Font("黑体", Font.PLAIN, 40); // 通过Font构造字体
					g.setFont(mFont); // 设置字体
					g.drawString(content[i], x + 40, y);
					break;
				case 2:
					mFont = new Font("黑体", Font.PLAIN, 36); // 通过Font构造字体
					g.setFont(mFont); // 设置字体
					g.drawString(content[i], x + 25, y);
					break;
				case 3:
					mFont = new Font("黑体", Font.PLAIN, 30); // 通过Font构造字体
					g.setFont(mFont); // 设置字体
					g.drawString(content[i], x + 20, y);
					break;
				case 4:
					mFont = new Font("黑体", Font.PLAIN, 28); // 通过Font构造字体
					g.setFont(mFont); // 设置字体
					g.drawString(content[i], x + 10, y);
				}
			} else {
				mFont = new Font("黑体", Font.PLAIN, 22); // 通过Font构造字体
				g.setFont(mFont); // 设置字体
				if (Math.ceil(contentLen / 5.0) == 1) {
					g.drawString(content[i], x, y);
				} else if (Math.ceil(contentLen / 5.0) == 2) {
					// 分两行写
					g.drawString(content[i].substring(0, 5), x, y);
					g.drawString(content[i].substring(5), x, y + 10);
				} else if (Math.ceil(contentLen / 5.0) == 3) {
					// 分三行写
					g.drawString(content[i].substring(0, 5), x, y - 10);
